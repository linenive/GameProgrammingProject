	...
	for (j = 0; j < tile_size_y; j++) {
		for (i = 0; i < tile_size_x; i++) {
			...
			tile_map[i + tile_size_x * j] = new Tile(
				Surface(SurfaceType(SURFACE_EARTH), createTransform2D(pos_x, pos_y), Vector2(TILE_WIDTH, TILE_HEIGHT))
			);
		}
	}

	for (i = 227; i < 232; i++) {
		tile_map[i]->SetSurfaceType(SurfaceType(SURFACE_RIVER));
	}

	tile_map[613]->SetSurfaceType(SurfaceType(SURFACE_RIVER));
	tile_map[766]->SetSurfaceType(SurfaceType(SURFACE_RIVER));

	for (i = 900; i < 1004; i++) {
		tile_map[i]->SetSurfaceType(SurfaceType(SURFACE_OCEAN));
	}
}